...
REM PP-OCRv6 small 检测 + 识别（移动端折中）；Windows 默认 1 worker（见 OCR_WORKERS 说明）
...
set OCR_DET_MODEL_NAME=PP-OCRv6_small_det

set OCR_REC_MODEL_NAME=PP-OCRv6_small_rec
...
echo [OCR] Profile:  PP-OCRv6 small (det+rec)
...
echo [OCR] Profile: PP-OCRv6 small  (background)

start "PaddleOCR-v6m" /D "%~dp0" /MIN "%PYEXE%" -m uvicorn app.main:app --host %OCR_HOST% --port %OCR_PORT% --workers %OCR_WORKERS% --log-config "%~dp0uvicorn_log_config.json"
