...
执行后会把 **锁定清单中的全部 wheel**（含 `paddlepaddle` 及 OCR 依赖）、**Paddle CPU 栈（写入 `offline_bundle/paddle`）**、PaddleOCR 模型下载到 `offline_bundle` 对应目录，并运行 **`scripts/verify_offline_bundle.py`** 自检。拷贝到内网后可在项目根执行 **`initServ.bat`** 初始化 `.venv`（详见 [服务器部署.md](服务器部署.md) **第 4.1 节**），或按同文档 **第 4.2 节** 手动 `pip`；离线包说明与拷贝前自检见 **第 3 节**；拷贝前也可单独执行 `python scripts/verify_offline_bundle.py`。
...
1) 安装 Python（版本需与 `offline_bundle\wheels` 中 wheel 的 `cp3xx` 一致）与 VC++ 运行库（见 [服务器部署.md](服务器部署.md) 第 2 节）  
2) **推荐**：在项目根执行 **`initServ.bat`** 一次，自动创建 `.venv` 并完成离线 `pip install`（无需联网）  
3) 模型目录使用项目内相对路径（默认无需改）  
4) 运行与需求匹配的启动脚本（如 **`startupv5s.bat`**，见 §13）并访问 `/health`

等价的手动命令（与 `initServ.bat` 内部一致）：
```powershell
cd E:\paddleOcr
python -m venv .venv
.venv\Scripts\activate

# 一条命令（锁定清单已含 paddlepaddle 及传递依赖；双 find-links 与 verify_offline_bundle 顺序一致）
pip install --no-index --find-links .\offline_bundle\wheels --find-links .\offline_bundle\paddle -r .\offline_bundle\wheels\requirements-lock.txt
