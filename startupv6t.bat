...
REM PP-OCRv6 tiny 检测 + 识别（端侧轻量）；Windows 默认 1 worker（见 OCR_WORKERS 说明）
...
set OCR_DET_MODEL_NAME=PP-OCRv6_tiny_det

set OCR_REC_MODEL_NAME=PP-OCRv6_tiny_rec
...
echo [OCR] Profile:  PP-OCRv6 tiny (det+rec)
...
echo [OCR] Profile: PP-OCRv6 tiny  (background)

start "PaddleOCR-v6t" /D "%~dp0" /MIN "%PYEXE%" -m uvicorn app.main:app --host %OCR_HOST% --port %OCR_PORT% --workers %OCR_WORKERS% --log-config "%~dp0uvicorn_log_config.json"
