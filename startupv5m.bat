...
set OCR_DET_MODEL_NAME=PP-OCRv5_mobile_det
...
echo [OCR] Profile:  PP-OCRv5 mobile (det+rec)
...
echo [OCR] Profile: PP-OCRv5 mobile  (background)

start "PaddleOCR-v5m" /D "%~dp0" /MIN "%PYEXE%" -m uvicorn app.main:app --host %OCR_HOST% --port %OCR_PORT% --workers %OCR_WORKERS% --log-config "%~dp0uvicorn_log_config.json"
